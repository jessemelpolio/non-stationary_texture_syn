...
img_number = 34
results_dir = './datasets/half/{}/perlin'.format(img_number)
src_dir = os.path.join(results_dir, 'perlin_src')
merge_dir = os.path.join(results_dir, 'merge_with_perlin')
...
another_src_path = "./datasets/half/{}/train/{}.jpg".format(img_number, img_number)
another_img = PIL.Image.open(another_src_path)
width, height = another_img.size
another_src_ndarray = cv2.imread(another_src_path)

width_res = 30
height_res = 30
frames = 20
frameres = 5
w_space_range, h_space_range = width//width_res, height//height_res
frame_range = frames//frameres

pnf = PerlinNoiseFactory(3, octaves=4, tile=(w_space_range, h_space_range, frame_range))

for t in range(frames):
    img = PIL.Image.new('L', (width, height))
    for x in range(width):
        for y in range(height):
            n = pnf(x/float(width_res), y/float(height_res), t/float(frameres))
            img.putpixel((x, y), int((n + 1) / 2 * 255 + 0.5))
    dst = np.zeros((height, width, 3))
    img_convert_ndarray = np.zeros((height, width, 3))
    img_convert_ndarray[:,:,0] = np.array(img)
    img_convert_ndarray[:,:,1] = np.array(img)
    img_convert_ndarray[:,:,2] = np.array(img)
    # print(img_convert_ndarray)

    dst = 0.5 * another_src_ndarray + 0.5 * img_convert_ndarray 
    # np.transpose(dst, (1, 2, 0))
    # print(dst)
    dst = np.uint8(dst)
    # print(dst)
    cv2.imwrite(os.path.join(merge_dir, "{}_{}_perlin.png".format(img_number ,t)), dst)
    # dst.save("withContent{:03d}.png".format(t))
    img.save(os.path.join(src_dir, "{}_noise_{:03d}.png".format(img_number, t)))
    print(t)